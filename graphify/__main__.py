...
_LEGACY_CODEX_USER_PROMPT_HOOK = {
    "hooks": [
        {
            "type": "command",
            "command": (
...
def _is_graphify_codex_hook(hook: object) -> bool:
    ...
    return hook in (
        _CODEX_HOOK["hooks"]["PreToolUse"][0],
        _LEGACY_CODEX_USER_PROMPT_HOOK,
        _LEGACY_CODEX_PRE_TOOL_HOOK,
    ) or "graphify hook-check" in str(hook)
...
def _install_codex_hook(project_dir: Path) -> None:
    ...
    if pre_tool is None:
        ...
        print("  .codex/hooks.json  ->  PreToolUse hooks are not a list; left unchanged")
        return
...
def main() -> None:
    ...
    if cmd == "install":
        ...
        provider = None
        runner = None
        include_wiki = "--no-wiki" not in argv[3:]
        include_html = "--no-html" not in argv[3:]
        args = argv[3:]
        i = 0
        while i < len(args):
            if args[i] == "--provider" and i + 1 < len(args):
        ...
        if not is_kb_root(watch_path):
            ...
            ok = _rebuild_code(watch_path, force=force) if force else _rebuild_code(watch_path)
            if ok:
